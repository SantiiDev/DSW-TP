...
* Se agrega la entidad **`LISTS`** (colección personal de álbumes armada por un usuario) para el alcance adicional voluntario "Listas personalizadas". Es siempre pública, tal como quedó definida en el alcance funcional: no lleva ninguna columna de visibilidad. Sus álbumes son una relación N:M que se materializa como la tabla intermedia **`LIST_ALBUMS`**, con clave primaria compuesta `(id_list, id_album)` y dos atributos propios: `added_date` y `position`, que le da a la lista un orden propio (lo asigna el service como el siguiente entero libre; no hay pantalla para reordenar a mano). Se agrega además la relación N:M **`LIST_LIKES`** entre `USERS` y `LISTS`, con el mismo argumento que `REVIEW_LIKES`: un contador no registra *quién* reaccionó, y sin ese dato no se puede impedir el doble "me gusta" ni retirarlo. Se materializa con el mismo tratamiento que `REVIEW_LIKES`: tabla intermedia con clave primaria compuesta `(id_list, id_user)` y el atributo propio `liked_date`.
