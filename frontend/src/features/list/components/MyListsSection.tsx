  ...
  return (
    <section className="my-lists">
            ...
            {visibleLists.map((list) => (
              <li key={list.id}>
                <Link to={`/lists/${list.id}`} className="my-lists__item">
                  ...
                  <span className="my-lists__item-count">{list.albumsLabel}</span>
                </Link>
              </li>
            ))}
          </ul>
      ...
      <FormModal
        isOpen={isFormOpen}
        title="Crear lista"
        hint="Ponele un nombre y buscá los álbumes que la van a formar. Después podés seguir sumando desde la ficha de cada disco."
        error={formError}
        isBusy={isSubmitting}
        onClose={() => setIsFormOpen(false)}
      >
        <ListForm isSubmitting={isSubmitting} onSubmit={handleCreate} onCancel={() => setIsFormOpen(false)} />
      </FormModal>
    </section>
  );
};
